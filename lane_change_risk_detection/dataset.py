...
Original file is located at
    https://colab.research.google.com/drive/1tEKFPVqGWZiRDAEZxDsu4Xd7sgwdvfGZ
"""
...
class DataSet:
    ...
    def read_video(self, data_dir, option='fixed frame amount', number_of_frames=50, max_number_of_frames=500, scaling='no scaling', scale_x=0.1, scale_y=0.1):
        ...
        if option == 'fixed frame amount':
            self.video = np.zeros([len(foldernames), number_of_frames, self.image_seq[0].shape[0],
                                   self.image_seq[0].shape[1], self.image_seq[0].shape[2]])
        elif option == 'all frames':
            self.video = np.zeros([len(foldernames), max_number_of_frames, self.image_seq[0].shape[0],
                                   self.image_seq[0].shape[1], self.image_seq[0].shape[2]])
    ...
    def _read_video_helper(self, number_of_frames=50):
    ...
    def extract_features(self, img_path, feature_size=2048, option='fixed frame amount', number_of_frames=50,
        ...
        for foldername in tqdm(foldernames):
            if foldername.isnumeric:
                self.image_seq = self.load_images_for_torch(img_path + "/" + foldername)
                if not len(self.image_seq) == 0:
                    if option == 'fixed frame amount':
    ...
    def read_features(self, feature_path, feature_size=2048, option='fixed frame amount', number_of_frames=50,
                      max_number_of_frames=500):
    ...
    def read_risk_data(self, file_path):
        df = pd.read_csv(file_path, header=None, usecols=[3], names=['risk_score'])
    ...
    def load_images_for_torch(self, img_path, target_size=(224, 224)):
